...
class TRON {
  async createAccount(req, res) {
    try {
        ...
        const addressHash = keccak256(Buffer.from(publicKey, 'hex'));
        
        // Take the last 20 bytes of the hash and add the TRON prefix (41 in hex)
        const tronAddressHex = '41' + addressHash.substring(addressHash.length - 40);
        
        // Convert the hex address to Base58 to get the final TRON address
        const bytes = Buffer.from(tronAddressHex, 'hex');
  ...
  async createWallet(req, res) {
    try {
        ...
        // Derive the BIP32 root key from the seed and derive the TRON wallet path (same as Ethereum's)
        const root = hdkey.fromMasterSeed(seed);
        const addrNode = root.derive("m/44'/195'/0'/0/0"); // TRON's BIP44 path

        // Get the private and public key from the derived path
        ...
        // Add TRON prefix (41) and convert to Base58
        const tronAddressHex = '41' + addressHash.substring(addressHash.length - 40);
        const bytes = Buffer.from(tronAddressHex, 'hex');
        const tronAddress = bs58.encode(bytes);
      let data = {
